...
def parse_arguments(argv, prog=''):
    ...
    parser.add_argument('--k',
                        type=int,
                        default=1,
                        help='Number of nearest neighbour',
                        required=False)
    ...
    parser.add_argument('--init-nnf',
                        ...
                        required=False,
                        default=None)
    # Perform NLM denoising on the target image
    parser.add_argument('--nlm',
                        action='store_true',
                        help='Output the NLM-denoised target image',
                        required=False)
    # This is the h parameter in Section 3's third and fourth equations in
    # the Non-Local Means paper
    parser.add_argument('--nlm_h',
                        type=float,
                        help='Standard deviation of NLM algorithm',
                        default=0.5,
                        required=False)
...
def main(argv, prog=''):
    ...
    patmat.set_nlm(args.nlm)
    patmat.set_nlm_h(args.nlm_h)
    patmat.set_k(args.k)
    patmat.set_iterations(args.iters)
    patmat.set_patch_size(args.patch_size)
    patmat.set_alpha(args.alpha)
    patmat.set_w(args.w)
    patmat.set_init_NNF_k(nnf_file=args.init_nnf)
    # propagation and random search are enabled by default
    # so we toggle them to disable
    patmat.set_propagation(not args.disable_propagation)
    patmat.set_random(not args.disable_random)
